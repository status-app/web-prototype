...
type Props = { userResource: APIResource<User, []>; items: Accessor<Service[]> };
    ...
    {!props.userResource.val() ? ( // No user? Login. User? Show dashboard
      <div class="w-64 children:w-full">
        <div class="text-center">
          <span class="cursor-pointer" onClick={(e) => redirectToLogin("google")}>Google</span> •&nbsp;
          <span class="cursor-pointer" onClick={(e) => redirectToLogin("google")}>Twitter</span> •&nbsp;
          <span class="cursor-pointer" onClick={(e) => redirectToLogin("google")}>Discord</span> •&nbsp;
          <span class="cursor-pointer" onClick={(e) => redirectToLogin("google")}>Steam</span>
        </div>
        <hr class="mt-1" />
        {newUserResource.status() === "error" && <span class="text-red-500">Error: {newUserResource.error()}</span>}
        {newUserResource.status() === "fetching" && <span class="text-orange-500">Processing...</span>}
        {newUserResource.status() === "ok" && <><span class="text-green-500">Processing...</span></>}
        <form onSubmit={(ev) => doLogin(ev)} class="children:w-full mt-2">
          <div class="table children:table-cell">
            ...
            <input
              ...
              onKeyDown={(e) => e.key.toLowerCase() === "arrowdown" && document.getElementById("password")?.focus()}
            />
          </div>
          <div class="table children:table-cell mt-2">
            ...
            <input
              ...
              onKeyDown={(e) =>
                (e.key.toLowerCase() === "arrowup" && document.getElementById("login")?.focus())
                  || (e.key.toLowerCase() === "arrowdown" && document.getElementById("submit")?.focus())}
            />
          </div>
          <div class="text-center">
            <input
              ...
              type="submit"
              value="submit"
              onKeyDown={(e) =>e.key.toLowerCase() === "arrowup" && document.getElementById("password")?.focus()}
            />
          </div>
        </form>
      </div>
